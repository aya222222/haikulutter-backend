...
export const getProfile = async (req, res) => {
  ...
  try {
    ...
    console.log("profile is " + JSON.stringify(existingUser));
  } catch (error) {
...
export const createProfile = async (req, res) => {
  ...
  try {
    if (profileData?.profileBgImg || profileData?.profileIconImg) {
      ...
      bgImg = await cloudinary.uploader.upload(
        profileData?.profileBgImg,
        { timeout: 120000 },
        function (error, result) {}
      );
      iconImg = await cloudinary.uploader.upload(
...
export const updateProfile = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(oldProfile._id))
  ...
  try {
    ...
    if (profileData?.profileIconImg !== oldProfile?.profileIconImg) {
      ...
      newIconImg = await cloudinary.uploader.upload(
        profileData.profileIconImg,
        { timeout: 120000 },
        function (error, result) {}
      );

      //find all posts that user created and update the icon image of posts
      await PostHaiku.updateMany(
        { creator: userId },
        { $set: { profileIconImg: newIconImg?.secure_url } }
      ).lean();
    }
    ...
    if (profileData?.profileBgImg !== oldProfile.profileBgImg) {
      ...
      newBgImg = await cloudinary.uploader.upload(
        profileData.profileBgImg,
        { timeout: 120000 },
        function (error, result) {}
      );
...
export const followOrUnfollowCreator = async (req, res) => {
  ...
  try {
    ...
    const targetCreator = await Profile.findOne({
      userId: String(creator),
    }).lean();
    const index = loggedInUser.following.findIndex(
      (targetUser) => targetUser.userId == String(creator)
    );
    ...
    if (index === -1) {
      ...
      loggedInUser.following = loggedInUser.following.filter(
        (targetCreator) => targetCreator.userId !== String(creator)
      );
    }

    const updatedLoggedInUserProfile = await Profile.findByIdAndUpdate(
      loggedInUser._id,
      loggedInUser,
      { new: true }
    ).lean();
...
export const updateCreatorFollowStatus = async (req, res) => {
  ...
  try {
    ...
    const loggedInUser = await Profile.findOne({ userId }).lean();
    const index = creatorProfile.follower.findIndex(
    ...
    if (index === -1) {
      ...
      creatorProfile.follower = creatorProfile.follower.filter(
        (user) => user.userId !== String(userId)
      );
    }

    // const updatedLoggedInUserProfile = await Profile.findByIdAndUpdate(loggedInUser._id, loggedInUser, {new : true}).lean();
    const updatedCreatorProfile = await Profile.findByIdAndUpdate(
      creatorProfile._id,
      creatorProfile,
      { new: true }
    ).lean();
...
export const updateTargetCreatorFollowStatus = async (req, res) => {
  ...
  try {
    // const loggedInUser = await Profile.findOne({userId});
    const targetCreatorProfile = await Profile.findOne({
      userId: String(targetCreator),
    }).lean();
    const loggedInUser = await Profile.findOne({ userId }).lean();
    const index = targetCreatorProfile.follower.findIndex(
      (user) => user.userId === String(userId)
    );
    if (index === -1) {
      //followed by logged in user
      ...
      //unfollowed by logged in user
      // loggedInUser.following = loggedInUser.following.filter((creatorId) => creatorId !== creator);
      targetCreatorProfile.follower = targetCreatorProfile.follower.filter(
        (user) => user.userId !== String(userId)
      );
    }

    // const updatedLoggedInUserProfile = await Profile.findByIdAndUpdate(loggedInUser._id, loggedInUser, {new : true}).lean();
    const updatedTargetCreatorProfile = await Profile.findByIdAndUpdate(
      targetCreatorProfile._id,
